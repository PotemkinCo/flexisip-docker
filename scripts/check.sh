...
command -v rg >/dev/null || { echo 'error: rg is required' >&2; exit 1; }
...
if rg -n 'type=raw,value=latest|ghcr\.io/[^[:space:]]+:(latest)([[:space:]"`]|$)' .github README.md AGENTS.md docker-compose.yml; then
  echo 'error: mutable latest image references remain' >&2
  exit 1
fi
if rg -n 'build-debs|softprops/action-gh-release|CPACK_GENERATOR=DEB' .github/workflows; then
  echo 'error: .deb build/release workflow remains' >&2
  exit 1
fi
if rg -n 'ENABLE_EKT_SERVER' .env.example docker-compose.yml docker/proxy/entrypoint.sh docker/conference/entrypoint.sh config/flexisip-conference.conf; then
  echo 'error: obsolete runtime ENABLE_EKT_SERVER flag remains' >&2
  exit 1
fi
if rg -n "password='flexisip'|MARIADB_ROOT_PASSWORD=flexisip|MARIADB_PASSWORD=flexisip" .env.example docker-compose.yml config/flexisip-conference.conf; then
